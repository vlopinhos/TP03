...
class Lista {
    ...
    private void quicksort(int esq, int dir) {
        ...
        while (i <= j) {
            while (array[i].getSituation().compareTo(pivo.getSituation()) < pivo.getSituation().compareTo(array[i].getSituation())) {
                i++;
            }
            while (array[j].getSituation().compareTo(pivo.getSituation()) > pivo.getSituation().compareTo(array[j].getSituation())) {
                j--;
            }
            if (i <= j) {
                swap(i, j);
...
public class Filme {
    ...
    public static void main(String[] args) throws Exception {
        ...
        Arq.openWrite("746030_quicksort.txt.");
